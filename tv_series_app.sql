...
CREATE TABLE `tvseries_app`.`users` (
...
CREATE TABLE `tvseries_app`.`authorities` (
  ...
  CONSTRAINT `fk_authorities_user_id`
    FOREIGN KEY (`user_id`)
    REFERENCES `tvseries_app`.`users` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE);

INSERT INTO `tvseries_app`.`users` (`password`, `user_name`) VALUES ('$2a$04$KNLUwOWHVQZVpXyMBNc7JOzbLiBjb9Tk9bP7KNcPI12ICuvzXQQKG', 'admin');

INSERT INTO `tvseries_app`.`authorities` (`user_id`, `authority`) VALUES ('1', 'ADMIN');
